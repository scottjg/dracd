...
static void drac4_send_login(client_ctx *ctx)
{
	...
	md5((unsigned char *)ctx->passwd, strlen(ctx->passwd), digest1);
...
int incoming_data_ctrl(client_ctx *ctx, uint8_t *data, size_t len)
{
	...
	int r = 0;
	uint8_t *ptr = ctx->ctrl_buffer;

	while (ctx->ctrl_buffer_size >= 8) {
		if (ctx->dracType == DRAC4) {
			type = ntohl(*(uint32_t *)&ptr[4]);
			msg_size = ntohl(*(uint32_t *)&ptr[0]);
		} else {
			type = ntohs(*(uint16_t *)&ptr[4]);
			msg_size = ntohs(*(uint16_t *)&ptr[6]);
		}
		...
		if (memcmp(ptr, "APCP", 4) == 0)
			r = handle_init_response_ctrl(ctx, &ptr[8], type, msg_size - 8);
		else {
			if (ctx->dracType == DRAC4)
				r = drac4_handle_msg_ctrl(ctx, ptr, type, msg_size);
			else
				r = handle_msg_ctrl(ctx, &ptr[8], type, msg_size - 8);
		}
	...
	if (r >= 0) {
		if (msg_size == 0)
			msg_size = 8;
		assert(msg_size > ctx->ctrl_buffer_size);
		ctx->glue_read_data_ctrl(ctx, msg_size - ctx->ctrl_buffer_size);
	}

	if (ctx->ctrl_buffer_size > 0 && ptr != ctx->ctrl_buffer)
		memmove(ctx->ctrl_buffer, ptr, ctx->ctrl_buffer_size);
...
int incoming_data_video(client_ctx *ctx, uint8_t *data, size_t len)
{
	...
	uint8_t *ptr = ctx->video_buffer;
	while (ctx->video_buffer_size >= 8) {
		type = ntohs(*(uint16_t *)&ptr[4]);
		msg_size = ntohs(*(uint16_t *)&ptr[6]);
		...
		if (memcmp(ptr, "APCP", 4) == 0)
			r = handle_init_response_video(ctx, &ptr[8], type, msg_size - 8);
		else
			r = handle_msg_video(ctx, &ptr[8], type, msg_size - 8);
	...
	if (ctx->video_buffer_size > 0 && ptr != ctx->video_buffer)
		memmove(ctx->video_buffer, ptr, ctx->video_buffer_size);
...
static int handle_init_response_ctrl(client_ctx *ctx, uint8_t *data, uint16_t type, uint16_t msg_size)
{
	...
	if(data[9] & 0x4) {
		printf("switching on ssl for ctrl socket\n");
	...
	if (ctx->dracType == C6000 || ctx->dracType >= DRAC7)
		ctx->glue_write_data_ctrl(ctx, "\x00", 1); //causes auth to fail on drac7+
	else
...
static int handle_init_response_video(client_ctx *ctx, uint8_t *data, uint16_t type, uint16_t msg_size)
{
	//switch to ssl if the server wants it
	if(data[9] & 0x4) {
		printf("switching to ssl for video\n");
...
static int handle_msg_video(client_ctx *ctx, uint8_t *data, uint16_t type, uint16_t msg_size)
{
	...
	switch (type) {
		case TYPE_VIDEO_CONNECT_STATUS:
			...
			if (status != 0) {
				printf("video connect failed with status: %"PRIu64"\n", status);
				return -1;
			}
		...
		default:
			status = *(uint64_t *)data;
			//printf("unhandled video packet type %d (size=%hu): %"PRIu64"\n", type, msg_size, status);
			break;
	}
...
static int drac4_handle_msg_ctrl(client_ctx *ctx, uint8_t *data, uint32_t type, uint32_t msg_size)
{
	...
	switch (msgtype) {
		case TYPE_DRAC4_LOGIN_RESPONSE:
			...
			//if the screen is blanked, we won't get a frame, so put a placeholder black frame here
			init_framebuffer(ctx, 480, 640);
			return NEW_FRAME_READY;
			break;
